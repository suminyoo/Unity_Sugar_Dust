...
public class TransactionUI : MonoBehaviour
{
    ...
    public TextMeshProUGUI itemTotalPriceText;

    public TextMeshProUGUI playerZoneTotalText;
    public TextMeshProUGUI currentTotalSalesText;
    ...
    [Header("Money")]
    public Transform customerMoneyBoard;
    ...
    public void UpdateTotalSalesUI()
    {
        if (currentTotalSalesText == null) return;

        int currentSales = 0;

        currentSales = SalesManager.Instance.totalSales;
        
        currentTotalSalesText.text = $"{currentSales} {CustomerPaymentSystem.CURRENCY_SYMBOL}";
    }
    ...
    private void SpawnMoneyInternal(int amount, RectTransform parentZone, List<DraggableMoney> targetList, float fakeProb, bool isMine, bool isScattered, int index)
    {
        ...
        DraggableMoney dm = moneyObj.GetComponent<DraggableMoney>();
        ...
        // 초기화
        dm.Initialize(amount, spriteToUse, data.size, dragLayer, isMine, parentZone, (deletedMoney) => {
    ...
    private void OnClickConfirm()
    {
        ...
        //위조지폐 리스트
        List<int> acceptedFakeBills = new List<int>();
        foreach (var money in customerMonies)
        {
            if (money.isFake)
            {
                acceptedFakeBills.Add(money.amount);
            }
        }
